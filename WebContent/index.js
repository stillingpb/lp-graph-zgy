		...
		var that = {
			init : function() {
				...
				$.each(_maps, function(stub, map) {
					_links.append("<li><a href='#/" + stub + "' class='" + stub
							+ "'>" + map.title + "</a></li>")
				})
			...
			selectMap : function(map_id) {
				...
				// 获取服务端的数据
				var url = "maps/" + map_id + ".json";
				if (map_id == "Json") {
					url = "/HtmlMap";
				}
				$.getJSON(url, function(data) {
					// load the raw data into the particle system as is (since
					// it's already formatted correctly for .merge)
